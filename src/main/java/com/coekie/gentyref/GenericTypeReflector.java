...
public class GenericTypeReflector {
   ...
   * @param typeAndParams must be either ParameterizedType, or (in case there are no type arguments,
   *     or it's a raw type) Class
  ...
  /**
   * Checks if the given type is a class that is supposed to have type parameters, but doesn't. In
   * other words, if it's a really raw type.
   */
  ...
  /**
   * Returns a type representing the class, with all type parameters the unbound wildcard ("?"). For
   * example, <tt>addWildcardParameters(Map.class)</tt> returns a type representing
   * <tt>Map&lt;?,?&gt;</tt>.
   *
   * @return
   *     <ul>
   *       <li>If clazz is a class or interface without type parameters, clazz itself is returned.
   *       <li>If clazz is a class or interface with type parameters, an instance of
   *           ParameterizedType is returned.
   *       <li>if clazz is an array type, an array type is returned with unbound wildcard parameters
   *           added in the the component type.
   *     </ul>
   */
  ...
  /**
   * Finds the most specific supertype of <tt>type</tt> whose erasure is <tt>searchClass</tt>. In
   * other words, returns a type representing the class <tt>searchClass</tt> plus its exact type
   * parameters in <tt>type</tt>.
   *
   * <ul>
   *   <li>Returns an instance of {@link ParameterizedType} if <tt>searchClass</tt> is a real class
   *       or interface and <tt>type</tt> has parameters for it
   *   <li>Returns an instance of {@link GenericArrayType} if <tt>searchClass</tt> is an array type,
   *       and <tt>type</tt> has type parameters for it
   *   <li>Returns an instance of {@link Class} if <tt>type</tt> is a raw type, or has no type
   *       parameters for <tt>searchClass</tt>
   ...
   * <tt>getExactSuperType(StringList.class, Collection.class)</tt> returns a {@link
   * ParameterizedType} representing <tt>Collection&lt;String&gt;</tt>.
   */
  ...
  /**
   * Gets the type parameter for a given type that is the value for a given type variable. For
   * example, with <tt>class StringList implements List&lt;String&gt;</tt>,
   * <tt>getTypeParameter(StringList.class, Collection.class.getTypeParameters()[0])</tt> returns
   * <tt>String</tt>.
   *
   * @param type The type to inspect.
   * @param variable The type variable to find the value for.
   * @return The type parameter for the given variable. Or null if type is not a subtype of the type
   *     that declares the variable, or if the variable isn't known (because of raw types).
   */
  ...
  /**
   * If type is an array type, returns the type of the component of the array. Otherwise, returns
   * null.
   */
  ...
  private static Type[] getArrayExactDirectSuperTypes(Type arrayType) {
    ...
    if (typeComponent instanceof Class && ((Class<?>) typeComponent).isPrimitive()) {
      ...
      for (resultIndex = 0; resultIndex < componentSupertypes.length; resultIndex++) {
        result[resultIndex] =
            GenericArrayTypeImpl.createArrayType(componentSupertypes[resultIndex]);
      }
    }
   ...
   * <tt>m.getGenericReturnType()</tt> when the method was declared in a superclass, or
   * <tt>type</tt> has a type parameter that is used in the return type, or <tt>type</tt> is a raw
   * type.
   */
  ...
  /**
   * Returns the exact type of the given field in the given type. This may be different from
   * <tt>f.getGenericType()</tt> when the field was declared in a superclass, or <tt>type</tt> has a
   * type parameter that is used in the type of the field, or <tt>type</tt> is a raw type.
   */
  ...
  /**
   * Returns the exact parameter types of the given method in the given type. This may be different
   * from <tt>m.getGenericParameterTypes()</tt> when the method was declared in a superclass, or
   * <tt>type</tt> has a type parameter that is used in one of the parameters, or <tt>type</tt> is a
   * raw type.
   */
  ...
  /**
   * Returns list of classes and interfaces that are supertypes of the given type. For example given
   * this class: <tt>class Foo&lt;A extends Number & Iterable&lt;A&gt;, B extends A&gt;</tt><br>
   ...
   *
   * <p>This is mostly useful if you get a type from one of the other methods in
   * <tt>GenericTypeReflector</tt>, but you don't want to deal with all the different sorts of
   * types, and you are only really interested in concrete classes and interfaces.
   *
   * @return A List of classes, each of them a supertype of the given type. If the given type is a
   *     class or interface itself, returns a List with just the given type. The list contains no
   *     duplicates, and is ordered in the order the upper bounds are defined on the type.
   */
